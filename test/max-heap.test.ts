...
describe('[Max Heap] 静态方法 - fromArray 方法', () => {
  let a, arr;
  beforeEach(() => {
    arr = [1, 2, 3, chance.integer({ min: 4, max: 10 })]; // 注意最小堆的排序顺序
    a = MaxHeap.fromArray(arr);
  });

  test('返回堆的第一个元素（头元素）', () => {
    expect(a.peek).toBe(arr[3]);
    expect(a.length).toBe(arr.length);
    expect(a.isEmpty()).toBeFalsy();
  });
});

describe('[Max Heap] 静态方法 - toString 方法', () => {
  let a,
    arr = [2, 7, 26, 25, 19, 17, 1, 90, 3, 36];
  const expectHeapArray = [90, 36, 17, 25, 26, 7, 1, 2, 3, 19];
  beforeEach(() => {
    a = MaxHeap.fromArray(arr);
  });

  test('返回构建完毕后的数组', () => {
    expect(a.peek).toBe(90);
    expect(a.toString()).toBe(expectHeapArray.toString());
  });
});

describe('[Max Heap] - 左侧子节点方法', () => {
  let a,
    arr = [2, 7, 26, 25, 19, 17, 1, 90, 3, 36];
  const expectHeapArray = [90, 36, 17, 25, 26, 7, 1, 2, 3, 19];
  beforeEach(() => {
...
describe('[Max Heap] - poll 方法', () => {
  ...
  test('poll 返回顶部元素，下滤', () => {
    expect(a.poll()).toBe(90);
    expect(a.toString()).toBe([36, 26, 17, 25, 19, 7, 1, 2, 3].toString());

    expect(a.poll()).toBe(36);
    expect(a.toString()).toBe([26, 25, 17, 3, 19, 7, 1, 2].toString());
  });
...
describe('[Max Heap] - remove 方法', () => {
  ...
  test('移除单个元素', () => {
    ...
    expect(a.toString()).toBe([25, 7, 19, 1, 3, 7].toString());
  });

  test('移除重复元素', () => {
    a.remove(7);
    expect(a.length).toBe(arr.length - 2);
    expect(a.toString()).toBe([25, 3, 19, 1, 2].toString());
  });
});
